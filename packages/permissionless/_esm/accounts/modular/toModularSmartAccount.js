 ...
 */
export async function toModularSmartAccount(parameters) {
    ...
    const encodeExecution = (calls) => {
        if (calls.length === 1) {
            ...
            const call = calls[0];
            return encodePacked(["address", "uint256", "bytes"], [call.to, call.value, call.data]);
        }
    ...
    const account = await toSmartAccount({
        ...
        async encodeCalls(calls) {
            ...
            const callType = normalizedCalls.length === 1 ? CALLTYPE_SINGLE : CALLTYPE_BATCH;
